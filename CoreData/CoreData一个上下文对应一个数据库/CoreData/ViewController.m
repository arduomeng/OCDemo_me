...
- (void)viewDidLoad {
    ...
    _ctx = [self createManagedObjectContext:@"CoreData"];
...
- (NSManagedObjectContext *)createManagedObjectContext:(NSString *)name{
    ...
    NSLog(@"%@", path);
    NSError *error = nil;
    [persistentStroe addPersistentStoreWithType:NSSQLiteStoreType configuration:nil URL:[NSURL fileURLWithPath:path] options:nil error:&error];
    ...
    // 3. 设置上下文的持久存储器
    ctx.persistentStoreCoordinator = persistentStroe;

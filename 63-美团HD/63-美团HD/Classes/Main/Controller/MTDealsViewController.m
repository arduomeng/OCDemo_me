...
{
    //判断请求是否为最后一次的请求，若不是则说明，返回的是前一次请求的数据，直接return
...
- (void)request:(DPRequest *)request didFailWithError:(NSError *)error
{
    ...
    // 1.提醒失败（showError默认显示在window上，不会随着控制器的旋转而选择，所以显示在控制器的view上面）
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    ...
    /*
     * 只要刷新数据，都会来到这里，所以可以将一些判断条件放着数据源方法中，实时响应
     */
    
    
    // 计算一遍内边距(只要刷新表格，就判断一次屏幕方向)
    [self viewWillTransitionToSize:CGSizeMake(collectionView.width, 0) withTransitionCoordinator:nil];
    
    // 控制尾部刷新控件的显示和隐藏(只要刷新表格，就判断一次是否显示下拉刷新)
    self.collectionView.footerHidden = (self.totalCount == self.deals.count);
    
    // 控制"没有数据"的提醒(只要刷新表格，就判断一次是否显示"没有数据"的提醒)
